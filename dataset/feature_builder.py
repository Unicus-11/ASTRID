...
--------------------------------------------------------------------------
v0.5 -- RESOLUTION-SAFETY CORRECTNESS PASS (this revision)
--------------------------------------------------------------------------
The shared observation grid (camera_timeseries.csv,
gps_p{TAG}_timeseries.csv, assembled_observations_p{TAG}.csv) remains on
its existing SAMPLING_INTERVAL_S = 5 seconds cadence -- that constant is
unchanged. Nothing about the FEATURE DEFINITIONS, the file's architecture,
or the sampling interval itself changed in this revision. Cheng-style
per-probe critical-point extraction is explicitly OUT OF SCOPE and is not
added here.

This revision instead hardens the file against SAMPLING_INTERVAL_S ever
being read as a hardcoded "5" in disguise, so a future change to that
constant (in trajectory_utils.py) can't silently produce mislabeled or
wrong features here without this file raising or being re-audited first.

What already did NOT need any change, and why:
  - add_change_features()'s delta_steps = window_s // SAMPLING_INTERVAL_S
    already derives the step count from the shared constant rather than a
    hardcoded "5", so a "_change_30s" column spans 30 real seconds of
    history (6 steps at the current 5-second grid) with no code change.
  - add_occupancy_fraction(), add_signal_features()'s consecutive-red-
    streak run-length logic, and compute_k_jam() are all resolution-
    agnostic: they operate on values/timestamps, not on an assumed step
    duration.

What DID change (defensive correctness fixes, not a resolution change):
  1. add_change_features() now VALIDATES that window_s is an exact
     multiple of SAMPLING_INTERVAL_S and raises rather than silently
     mislabeling a column. At the current 5-second grid, 30 // 5 == 6 is
     exact, so this check passes silently today -- but it guards against
     a future SAMPLING_INTERVAL_S value for which that division would
     truncate, which would otherwise produce a "_change_30s" column that
     doesn't actually span 30 seconds without any error being raised.
     Failing loudly is safer than shipping a column whose name lies about
     what it measures.
  2. add_physics_features()'s empirical queue-front propagation rate
     (estimated_queue_front_propagation_m_per_s) previously divided the
     30-second change column by the LITERAL constant DELTA_WINDOW_S. That
     is only correct if the change column's actual elapsed time equals
     DELTA_WINDOW_S, which is now computed and asserted via (1) rather
     than assumed. The rate calculation now divides by the VERIFIED
     elapsed seconds for that column instead of the bare constant, so it
     stays correct even if DELTA_WINDOW_S or SAMPLING_INTERVAL_S changes
     again later.
  3. Docstrings/comments that described the grid as a fixed "5-second"
     cadence in prose throughout this file are corrected to describe it
     in terms of the SAMPLING_INTERVAL_S constant instead, so the prose
     can't drift out of sync with the constant's actual value again.
  4. red_duration_s (the consecutive-red-streak length) and
     estimated_hidden_queue_extension_m's held-rate extrapolation
     (rate * red_duration_s) both already operate in real seconds via
     timestamp subtraction, not step counts, so they automatically
     benefit from finer resolution without any code change -- called out
     here explicitly since it was verified, not assumed.

No feature was added, removed, or redefined in this revision. No Cheng
critical-point logic, per-probe trajectory consumption, or new ML
architecture was introduced.

--------------------------------------------------------------------------
v0.5 -- LITERATURE VERIFICATION PASS (prior revision)
--------------------------------------------------------------------------
...
earlier code comments claimed those papers said. Result, paper by paper:
  ...
  reports AGGREGATE probe statistics per approach edge on the shared
  observation grid, not individual probe position/speed/acceleration
  traces keyed by probe id across consecutive intervals). This is not an
  ...
  Note: moving the shared grid to 5-second resolution in this revision
  does NOT by itself provide that per-probe trajectory -- gps_simulator.py
  still reports an aggregate across however many probes are present each
  interval, not a probe-keyed trace -- so this conclusion is unchanged.
...
reason stated in its own section below.
...
Removed (as not observable by ASTRID's actual sensor outputs):
    ...
    - entry_flow_veh_per_hour / discharge_flow_veh_per_hour (required
      per-second boundary-crossing events; camera reports a snapshot)
    - probe_critical_point_count / probe_shockwave_sighting_m /
      probe_discharge_cp_count / probe_discharge_sighting_m (required
      per-second speed traces for identified probes; GPS reports
...
FULL TEXT READ DIRECTLY. Section/equation numbers below refer to that text.
  - Section 2, eq (1): q = n/tau -- flow as a boundary-crossing vehicle
    count per unit time. NOT used in this file: the camera's observation
    grid does not report crossing events, only region occupancy + speed.
  - Section 2, eq (2): k = (sum of vehicle-crossing-times)/(tau*dx) --
    density as a time-averaged quantity over a road slice.
    estimated_density_k_veh_per_km below is an ADAPTATION: an
    instantaneous count/length snapshot over the camera's own 150m
    visible region, not LW's time-averaged slice quantity, and not a
    true total-link density (see point 7 below).
  - Section 2, eq (3): v = q/k, the space-mean speed -- algebraically
    this is the same relationship as q = k*v. observed_flow_veh_per_hour
    below uses this identity, fed with ASTRID's own density/speed
    approximations (not LW's rigorously time-averaged q and k), and is
    explicitly an AGGREGATE, MIXED-REGIME estimate over whatever mix of
    free-flow and queued vehicles the camera currently sees -- not LW's
    clean single-regime q(k) functional relationship.
  - Section 2 eq (6)/(7), Section 3: wave speed c = dq/dk; shock
    (discontinuous wave) speed = the SLOPE OF THE CHORD joining two
    (k,q) points on the flow-concentration curve, delta q / delta k.
    This chord construction is theoretically correct for the red-light
    shock, but REQUIRES a clean upstream traffic state as one of the two
    chord endpoints -- see point below.
  - Section 6, p.338: describes the red-signal shock wave sent back into
    the oncoming stream, with speed given by the slope of the chord
    joining the oncoming-flow point to (k_j, 0). This is the LW
    red-signal shock construction. It is NOT implemented as a flux-based
    formula in this file (see v0.4 point 1 above) because ASTRID's
    camera cannot isolate the required "oncoming flow" (upstream-only)
    state from its mixed visible-region aggregate.
  ...
  - Section 6, eq (17): the EXACT shock position over time for the
    red-signal case. estimated_hidden_queue_extension_m is a first-order,
    held-rate approximation of the same physical picture (v0.4 point 3),
    not their exact integral.
...
conference paper on the identical method WAS read in full. Per that text:
  - A probe's trajectory is a series of (location, speed, acceleration)
    ...
    changes (their Eq.(1)-(3)) -- this REQUIRES a continuous per-vehicle
    trajectory, not an aggregated snapshot.
  ...
  - Their maximum-queue-length formula (Eq.(11)):
    ...
    trajectory, not from a population aggregate.

  WHY THIS IS NOT IMPLEMENTED IN ASTRID (as of this revision):
  gps_simulator.py's actual output remains an AGGREGATE across however
  many probes are present on an approach that interval (probe_count,
  probe_mean_speed_mps, probe_min/max_distance_to_stopline_m) -- moving
  that grid to 5-second resolution in this revision does not change this:
  there is still no continuous per-vehicle trajectory keyed by probe id
  to run CP extraction against, and no way to identify "the one probe
  whose Type II critical point this is." This is confirmed directly from
  Cheng et al.'s own method description, not inferred. An EARLIER version
  of this file's Cheng-inspired feature used probe_max_distance_to_stopline_m
  (the farthest of possibly several probes seen in a bin) as a substitute
  for their L_CP2, and red_duration_s as a substitute for (T_CP2 - T_r) in
  a conservation-style re-derivation that is structurally identical in
  FORM to their Eq.(11)/(12). Having now read the actual method, that
  substitution is a materially different and coarser measurement than
  what Cheng et al.'s equations require -- an aggregate maximum across an
  unknown number of probes is not the same quantity as one specific
  probe's precisely-timestamped critical-point position. Given the strict
  sensor-realism rule, this feature remains REMOVED rather than kept as a
  labeled-but-shaky adaptation. If gps_simulator.py is ever intentionally
  upgraded to expose per-probe trajectories (position/speed at fine
  resolution, keyed by probe id, across consecutive intervals), Cheng's
  actual Eq.(9)-(12) could be implemented as a separate, real baseline at
  that point -- not before, and not in this revision.
...
Reads (per scenario):
    ...
    All of the above are now on the shared SAMPLING_INTERVAL_S (5-second)
    grid; see the
    v0.5 note above.
...
def _resolve_delta_steps(window_s: int) -> int:
    """v0.5: resolution-safe conversion from a requested history window
    (in seconds) to a step count on the shared SAMPLING_INTERVAL_S grid.

    At the current 5-second grid, 30 // 5 == 6 was exact by
    construction and nobody had to think about it. Now that
    SAMPLING_INTERVAL_S is a variable (5 seconds in this revision, but not
    guaranteed to stay that way), silently truncating via `//` could
    produce a "_change_30s" column that actually spans fewer than 30
    real seconds -- a mismatch between the column's name and what it
    measures. This raises loudly instead, so a bad combination of
    DELTA_WINDOW_S / SAMPLING_INTERVAL_S is caught immediately rather
    than silently mislabeling a feature."""
    if window_s % SAMPLING_INTERVAL_S != 0:
        raise ValueError(
            f"window_s={window_s} is not an exact multiple of "
            f"SAMPLING_INTERVAL_S={SAMPLING_INTERVAL_S} -- refusing to build a "
            f"'_change_{window_s}s' column that would not actually span {window_s} "
            f"real seconds of history."
        )
    return max(1, window_s // SAMPLING_INTERVAL_S)
...
def add_change_features(df: pd.DataFrame, columns: List[str], window_s: int = DELTA_WINDOW_S) -> pd.DataFrame:
    ...
    v0.5: delta_steps is now resolved via _resolve_delta_steps(), which
    raises if window_s isn't an exact multiple of SAMPLING_INTERVAL_S,
    rather than silently truncating (see that function's docstring)."""
    df = df.sort_values(["approach_edge", "timestamp"]).copy()
    delta_steps = _resolve_delta_steps(window_s)
...
def add_signal_features(df: pd.DataFrame, tls_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    # This operates on real timestamp subtraction, not a step count, so it
    # is already correct at any SAMPLING_INTERVAL_S -- including the 1s
    # grid introduced in this revision -- with no change needed.
...
def compute_k_jam(scenario: dict, vehicle_types_cfg: dict, effective_gap_m: float) -> float:
    ...
    architecture rules prohibit. This is timestamp-resolution-independent.
    """
...
def add_physics_features(
    ...
    C. estimated_queue_front_propagation_m_per_s
        ...
        change of visible_queue_length_m over the change column's
        VERIFIED elapsed time (see v0.5 note below), not a bare literal.
        Positive = queue growing (front moving further from the stop
        line, i.e. propagating upstream). Valid only while
        queue_reaches_camera_edge is False for the current row. NOT
        sourced from any paper's equation -- a direct kinematic
        measurement of an observed boundary, standing in for the LW
        section 6 shock-speed concept without needing LW's
        clean-upstream-state assumption.

        v0.5 fix: previously divided by the bare constant DELTA_WINDOW_S.
        That is only correct if the "_change_{DELTA_WINDOW_S}s" column
        actually spans DELTA_WINDOW_S real seconds, which is now
        guaranteed (and checked) by _resolve_delta_steps() rather than
        assumed -- important once SAMPLING_INTERVAL_S is a variable
    ...
    D. estimated_hidden_queue_extension_m
        ...
        red_duration_s is real elapsed seconds (timestamp subtraction),
        so this is already correct at any grid resolution.
    ...
    docstring for the full reasoning. This revision's 5-second grid
    upgrade does not change that conclusion; Cheng-style per-probe CP
    extraction remains explicitly out of scope here.)
    """
    df = df.copy()
    ...
    # v0.5: verified elapsed seconds for change_col, rather than assuming
    # the bare DELTA_WINDOW_S constant is exactly right (see docstring).
    verified_window_s = _resolve_delta_steps(DELTA_WINDOW_S) * SAMPLING_INTERVAL_S
    ...
    if change_col in df.columns and "queue_reaches_camera_edge" in df.columns:
        raw_rate = pd.to_numeric(df[change_col], errors="coerce") / verified_window_s
...
def build_labels(scenario_dir: Path, horizon_s: Optional[int] = None) -> pd.DataFrame:
    ...
    if horizon_s:
        horizon_steps = _resolve_delta_steps(horizon_s)
...
def build_feature_manifest(layer: str, has_tls: bool) -> dict:
    manifest = {
        ...
        f"visible_queue_length_m_change_{DELTA_WINDOW_S}s": {"kind": "derived",
            "source": "camera history (past only)",
            "note": f"spans {DELTA_WINDOW_S} real seconds on the shared SAMPLING_INTERVAL_S="
                    f"{SAMPLING_INTERVAL_S}s grid"},
        f"visible_mean_speed_mps_change_{DELTA_WINDOW_S}s": {"kind": "derived",
            "source": "camera history (past only)",
            "note": f"spans {DELTA_WINDOW_S} real seconds on the shared SAMPLING_INTERVAL_S="
                    f"{SAMPLING_INTERVAL_S}s grid"},
    }
    ...
    if layer == "layer2":
        manifest.update({
            ...
            "estimated_queue_front_propagation_m_per_s": {"kind": "physics-derived (empirical)",
                ...
                "note": "rate of change of visible_queue_length_m over its VERIFIED elapsed window (see v0.5 "
                        "note); positive = queue growing upstream. Valid ONLY while queue_reaches_camera_edge "
                        "is False this interval; NA when censored"},
    ...
    manifest["_removed_features"] = {
        ...
        "entry_flow_veh_per_hour / discharge_flow_veh_per_hour": "were per-vehicle boundary-crossing counts "
            "(LW eq 1, q=n/tau); not producible from the camera's snapshot output.",
        "probe_critical_point_count / probe_shockwave_sighting_m / probe_discharge_cp_count / "
            ...
            "(read in full via its 2010 TRB precursor -- see module docstring) requires per-vehicle trajectory "
            "data keyed by probe id to detect a specific probe's critical point; gps_simulator.py exposes only "
            "AGGREGATE probe statistics, and this remains true even after the v0.5 5-second resolution "
            "(resolution and per-probe identity are independent properties). An earlier adaptation using the "
            "aggregate probe_max_distance_to_stopline_m field as a substitute for Cheng et al.'s L_CP2 was "
            "found, on reading their actual equations, to be a materially coarser and different quantity -- "
            "removed rather than kept as a mislabeled approximation.",
    ...
    manifest["_unimplemented_paper_methods"] = {
        ...
        "Cheng et al. (2012)": "full method understood via its 2010 TRB precursor; not implementable with "
            "ASTRID's current AGGREGATE GPS sensor (unaffected by the v0.5 5-second resolution) -- requires "
            "per-probe trajectories keyed by probe id.",
