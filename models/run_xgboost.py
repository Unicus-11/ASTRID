...
run_xgboost.py
=================
Small executable script that runs the existing XGBoost implementation
...
def main() -> None:
    ...
    # 1. XGBoost on Layer 1
    config_layer1 = ExperimentConfig(
        layer=Layer.LAYER1,
        random_state=RANDOM_STATE,
        experiment_name="xgboost_layer1_baseline",
    )
    model_layer1 = XGBoostModel(random_state=RANDOM_STATE)
    ...
    # 2. XGBoost on Layer 2 (p11)
    config_layer2 = ExperimentConfig(
        layer=Layer.LAYER2_P11,
        random_state=RANDOM_STATE,
        experiment_name="xgboost_layer2_p11_baseline",
    )
    model_layer2 = XGBoostModel(random_state=RANDOM_STATE)
