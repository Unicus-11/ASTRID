...
HistGradientBoosting baseline model.
...
persistence.py, model implementations) is modified.
"""
...
# models/results/error_analysis.py -> models/ is the parent's parent.
...
MODEL_FILE_NAME = "hist_gradient_boosting.joblib"
...
# These are the fixed, interpretable ranges requested. The script also
# prints the actual distribution (describe()) of true_queue_length_m for
# both splits so the reader can judge whether these bins are appropriate
# for the observed data; the underlying dataset is never modified.
# ---------------------------------------------------------------------------
TRUE_QUEUE_BIN_EDGES = [0, 25, 50, 100, 200, 400, np.inf]
TRUE_QUEUE_BIN_LABELS = ["0-25m", "25-50m", "50-100m", "100-200m", "200-400m", ">400m"]

# visible_queue_length_m: rather than assuming fixed edges (camera range
# is not known a priori from this script), quartile bins are computed
# from the ACTUAL observed test-split distribution at runtime. Edges are
# printed alongside the grouped results for transparency.
...
def load_baseline_consistency_row() -> Optional[pd.DataFrame]:
    ...
    match = baseline_df[
        (baseline_df["model"] == "hist_gradient_boosting")
...
def main() -> None:
    print("HISTGRADIENTBOOSTING LAYER 2 ERROR ANALYSIS")
    print("=" * len("HISTGRADIENTBOOSTING LAYER 2 ERROR ANALYSIS"))
